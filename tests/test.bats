...
@test "$SUITE: coverage run" {
  ...
  run env __GO_TEST_COVERAGE_RUN= TRAVIS_OS_NAME= "${test_cmd_argv[@]}"
  local IFS=$'\n'
  assert_success "${expected_kcov_args[*]}"
}

# This test also makes sure the invocation doesn't cause a second recursive call
# to `run_kcov` thanks to the `__GO_TEST_COVERAGE_RUN` variable.  Previously,
...
#
# With the `__GO_TEST_COVERAGE_RUN` variable, the recursive call is now
# short-circuited.
@test "$SUITE: run coverage by default on Travis Linux" {
  write_bats_dummy_stub_kcov_lib_and_copy_test_script
  create_test_go_script '@go "$@"'

  run env __GO_TEST_COVERAGE_RUN= TRAVIS_OS_NAME='linux' "$TEST_GO_SCRIPT" test
  assert_success
